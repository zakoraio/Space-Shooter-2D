...
Frame::Frame( const std::string& name, int len ,SDL_Surface* surf ) : 
  screen(IOManager::getInstance().getScreen()),
  surface( surf ),
  width(Gamedata::getInstance().getXmlInt(name.substr(0,len)+"/width")), 
  height(Gamedata::getInstance().getXmlInt(name.substr(0,len)+"/height")),
  sourceX(0),
  sourceY(0)
{ }
...
void Frame::draw(Sint16 x, Sint16 y, double angle) const {
  ...
  x -=  tmp->w/2 - surface->w/2;
  y -=  tmp->h/2 - surface->h/2;
